...
class Sincronizacion {
    ...
    public function obtenerRegistrosPendientesAgrupados($filtros = []) {
        ...
        foreach ($agrupados as &$registro) {
            ...
            // Calcular costo de horas normales SOLO de este registro
            foreach ($registro['detalles_normales'] as $normal) {
                $valor_hora = floatval($normal['valor_hora_base']);
                $horas = floatval($normal['horas_trabajadas']);
                $total_pagado += $valor_hora * $horas;
            }

            // Sumar costos de horas extras SOLO de este registro
            ...
            foreach ($registros as $registro) {
                ...
                if (!empty($registro['detalles_normales'])) {
                    foreach ($registro['detalles_normales'] as $detalle) {
                        try {
                            ...
                            $valor_hora = floatval($detalle['valor_hora_base']);
                            $horas = floatval($detalle['horas_trabajadas']);
                            $total_pagado_normal = $valor_hora * $horas;
                            ...
                            $stmt->execute([
                                ...
                                ':horas_extras' => 0,
                                ':total_pagado' => $total_pagado_normal,
                ...
                if (!empty($registro['detalles_extras'])) {
                    foreach ($registro['detalles_extras'] as $detalle) {
                        try {
                            // Calcular costo de horas extras con recargo
                            $costo_extra = $this->costos->calcularCostoHorasExtras(
                                $detalle['usuario_id'],
    ...
    public function obtenerEstadisticas() {
        ...
        return [
            ...
            'pendientes_total' => $pendientes_normales + $pendientes_extras,
